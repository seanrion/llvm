...
; RUN:     -riscv-save-csrs-early=false < %s \
; RUN:     | FileCheck %s --check-prefixes=ON
; RUN: llc -mtriple=riscv64 -mattr=+m -O2 -riscv-shrink-wrapping-dataflow=true \
; RUN:     -riscv-save-csrs-early=false -disable-riscv-frameless-ra < %s \
...
; RUN: llc -mtriple=riscv64 -mattr=+m -O2 -riscv-shrink-wrapping-dataflow=true \
; RUN:     -riscv-save-csrs-early=false -disable-riscv-frameless-ra < %s \
; RUN:     | FileCheck %s --check-prefixes=REALOFF
; RUN: llc -mtriple=riscv64 -mattr=+m -O2 -riscv-shrink-wrapping-dataflow=true \
; RUN:     -riscv-save-csrs-early=false < %s \
; RUN:     | FileCheck %s --check-prefixes=REALON
; RUN: llc -mtriple=riscv64 -mattr=+m -O2 -riscv-shrink-wrapping-dataflow=true \
; RUN:     -riscv-save-csrs-early=false \
