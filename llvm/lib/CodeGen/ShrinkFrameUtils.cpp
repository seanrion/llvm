    ...
    for (const MachineInstr &MI : MBB) {
      for (Register R : FrameBound) {
        if (R && MI.modifiesRegister(R, TRI)) {
          LLVM_DEBUG(dbgs() << "mustHoist check1: frame-bound "
                            << printReg(R, TRI) << " modified in "
...
bool llvm::shouldSkipFramelessRA(const MachineFunction &MF) {
  ...
  if (F.hasFnAttribute(Attribute::Naked) ||
      ...
      F.getCallingConv() == CallingConv::GHC || MF.hasEHFunclets())
    return true;
  ...
  if (!TFI->enableShrinkWrapping(MF) ||
      !TFI->enableCSRSaveRestorePointsSplit())
    return true;
