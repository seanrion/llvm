...
void PEIImpl::calculatePrologEpilogBlocks(MachineFunction &MF) {
  ...
  if (MFI.getProlog()) {
    ...
    if (Prolog != Entry &&
        mustHoistShrinkFramePrologToEntry(MF, Prolog, DT)) {
...
static void collectDominatedEpilogs(
    MachineBasicBlock *SaveBB, ArrayRef<MachineBasicBlock *> EpilogBlocks,
    const MachineDominatorTree &DT, SmallPtrSetImpl<MachineBasicBlock *> &Out) {
  ...
  if (!F.hasFnAttribute(Attribute::Naked)) {
    ...
    if (SplitPoints) {
      ...
      if (!MFI.getSavePoints().empty()) {
        ...
        // SP-relative CSR spills must not run before addi sp (bc_modulo / pr78622):
...
#ifndef NDEBUG
  // When shrink-frame is active, verify no FI in blocks outside the frame region.
