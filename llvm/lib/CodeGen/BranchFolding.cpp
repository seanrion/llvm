...
bool BranchFolder::CreateCommonTailOnlyBlock(MachineBasicBlock *&PredBB,
  ...
  // Jumping to the start of Prolog re-executes FrameSetup. After keepLegalSameTails
  // the preferred Dest is SameTails[0], but this routine may re-pick PredBB —
  // skip a whole-block Prolog so it cannot become the merge destination.
  MachineFunction &MF = *SameTails.front().getBlock()->getParent();
  MachineBasicBlock *Prolog =
      SFDomTree ? getShrinkFrameProlog(MF) : nullptr;
      ...
      if (MBB->empty()) {
        ...
        if (PredHasNoFallThrough || !PriorUnAnalyzable ||
          ...
          while(PI != MBB->pred_size()) {
            ...
            if (PMBB == MBB) {
              ...
              HasBranchToSelf = true;
                       ...
                       shrinkFrameGuardRejectEdge(MF, *PMBB, *CurTBB,
                                                  *SFDomTree)) {
              ++NumBranchOptsBlockedByFrameBoundary;
              ++PI;
            } else {
          ...
          if (DidChange) {
            ++NumBranchOpts;
            MadeChange = true;
            if (!HasBranchToSelf && MBB->pred_empty()) return MadeChange;
          }
        }
      }
