...
bool VarLocBasedLDV::isShrinkFrameInvalidOutsideFrame(
  ...
  const TargetLowering *TLI = MF.getSubtarget().getTargetLowering();
  Register SP = TLI->getStackPointerRegisterToSaveRestore();
  Register FP = TFI->hasFP(MF) ? TRI->getFrameRegister(MF) : Register();
...
void VarLocBasedLDV::transferShrinkFrameDestroy(MachineInstr &MI,
  ...
  const TargetLowering *TLI = MF.getSubtarget().getTargetLowering();
  Register SP = TLI->getStackPointerRegisterToSaveRestore();
  SmallVector<Register, 4> DeadRegs;
  DeadRegs.push_back(SP);
  if (TFI->hasFP(MF))
    DeadRegs.push_back(TRI->getFrameRegister(MF));
  ...
  for (LocIndex::u32_index_t Index : RegKill) {
    const VarLoc &VL = VarLocIDs[LocIndex(LocIndex::kUniversalLocation, Index)];
    emitUndef(VL);
  }
  if (!RegKill.empty())
