...
/// When shrink-frame is active, freeze only PEI-marked FrameSetup /
/// FrameDestroy MIs. Do not use isFrameRelatedForGuard here: that RISC-V hook
/// treats every SP/FP/ra access as frame-related and would kill
/// MakeCompressible's main size win (SP + large offset → scavenged base).
/// Matches MachineCopyPropagation for non-COPY MIs (MCP additionally freezes
/// frame-related COPYs only).
