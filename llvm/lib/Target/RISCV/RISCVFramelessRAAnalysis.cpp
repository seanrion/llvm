...
static bool usedInFramelessRegion(
    const SmallPtrSetImpl<MachineBasicBlock *> &FR, Register Reg) {
...
bool RISCVFramelessRAAnalysis::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (unsigned I = 0, E = MRI.getNumVirtRegs(); I != E; ++I) {
    ...
    MachineOptimizationRemarkAnalysis Remark(
        DEBUG_TYPE, "FramelessRAHint", DiagnosticLocation(DL), HintMBB);
    std::string Msg;
    raw_string_ostream(Msg) << "Frameless-path hint for " << printReg(Reg, TRI)
