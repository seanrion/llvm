...
/// Insertion context for NOPs immediately after a branch. Used by MC-layer
/// NopsAfterBranch cleanup to prioritize removal (OnExecPath first).
enum class NopsAfterBranchKind : uint8_t {
...
/// Fragment for NOPs immediately after a branch instruction. Variable-length:
/// can hold multiple consecutive NOPs. Filled via writeNopData during write.
class MCNopsAfterBranchFragment : public MCFragment {
  int64_t Size;
  NopsAfterBranchKind InsertKind;
  const MCSubtargetInfo &STI;

public:
  MCNopsAfterBranchFragment(int64_t NumBytes, NopsAfterBranchKind Kind,
                            const MCSubtargetInfo &STI)
      : MCFragment(FT_NopsAfterBranch, false), Size(NumBytes), InsertKind(Kind),
        STI(STI) {}

  int64_t getNumBytes() const { return Size; }
  void setNumBytes(int64_t Value) { Size = Value; }

  NopsAfterBranchKind getInsertKind() const { return InsertKind; }

  const MCSubtargetInfo *getSubtargetInfo() const { return &STI; }

  static bool classof(const MCFragment *F) {
    return F->getKind() == MCFragment::FT_NopsAfterBranch;
  }
};
} // end namespace llvm
